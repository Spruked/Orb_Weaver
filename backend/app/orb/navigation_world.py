...
def _build_pointer_registry(
    ...
    # A route+locator collision invalidates only the identities participating
    # in that collision. It must not disable unrelated verified targets.
    conflicted_target_ids = sorted({
        target_id
        for conflict in conflicts
        for target_id in conflict.get("target_ids", [])
    })
    for target_id in conflicted_target_ids:
        poi = pois.get(target_id)
        if not poi:
            continue
        poi["identity_conflict"] = True
        poi["guidance_eligible"] = False
        poi["guidance_block_reason"] = "ROUTE_LOCATOR_IDENTITY_CONFLICT"

    eligible = sum(
        1
        for poi in pois.values()
        if poi.get("guidance_eligible") is True
    )
    ...
    return {
        ...
        "conflicted_target_ids": conflicted_target_ids,
...
def _guidance_readiness(
    ...
    for route, target_ids in (pointer_registry.get("by_route") or {}).items():
        safe = sum(
            ...
            if _mapping(pois.get(target_id)).get("guidance_eligible") is True
        )
        route_status[route] = {
            "pointer_count": len(target_ids),
            "eligible_count": safe,
            "conflict_count": conflicts_by_route.get(route, 0),
            "status": "ELIGIBLE" if safe else "NO_ELIGIBLE_TARGET",
        }

    return {
        # Global map/deployment health remains visible and may still be BLOCKED.
        "status": status,
        "blockers": blockers,
        "map_recovery_required": recovery_required,
        "route_status": route_status,
        "planning_available": True,

        # Execution authority is target-scoped. A dirty global map must not
        # veto an otherwise verified, non-conflicted target.
        "pointer_execution_available": eligible_count > 0,
        "pointer_execution_scope": "route_target",
        "target_scoped_authority": True,
        "execution_policy": "target_scoped_pointer_authority_with_live_dom_verification",
    }
