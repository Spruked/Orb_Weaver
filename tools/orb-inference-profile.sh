...
case "${1:-status}" in
  gateway)
    start_bg gateway bash -lc "cd '$ROOT/backend' && exec '${PYTHON_BIN:-$ROOT/.venv/bin/python}' -m uvicorn inference_gateway:app --host 127.0.0.1 --port ${ORB_INFERENCE_GATEWAY_PORT:-16520}"
    ;;
  llama|llamacpp)
    stop_engines
