...
DEV_ROOT="${ORB_WEAVER_DEV_RUNTIME_ROOT:-}"
if [[ -z "${DEV_ROOT}" ]]; then
  DEV_ROOT="$(mktemp -d /tmp/orb-weaver-dev.XXXXXX)"
fi
LOG_ROOT="${DEV_ROOT}/logs"
PID_ROOT="${DEV_ROOT}/pids"

mkdir -p "${DEV_ROOT}/databases" "${LOG_ROOT}" "${PID_ROOT}" "${VAULT_ROOT}/development/runtime/tts_cache"
...
export PUBLIC_BASE_URL="${PUBLIC_BASE_URL:-http://127.0.0.1:16667}"
...
cd "${ROOT_DIR}/backend"
setsid "${BACKEND_PYTHON}" -m uvicorn main:app --host 127.0.0.1 --port "${BACKEND_PORT}" >"${LOG_ROOT}/backend-${BACKEND_PORT}.log" 2>&1 &
BACKEND_PID="$!"
echo "${BACKEND_PID}" >"${PID_ROOT}/backend-${BACKEND_PORT}.pid"

cd "${ROOT_DIR}/frontend"
setsid npm start >"${LOG_ROOT}/frontend-${FRONTEND_PORT}.log" 2>&1 &
FRONTEND_PID="$!"
