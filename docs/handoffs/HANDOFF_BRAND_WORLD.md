...
The canonical neutral identity is `orb_factory_default_v1`, the owner-supplied **O.R.B.S. Factory ORB** documented in [`../architecture/FACTORY_ORB_IDENTITY.md`](../architecture/FACTORY_ORB_IDENTITY.md). Every customer receives it first and may keep it indefinitely. It is Orb Weaver's own default identity, not detected customer branding.
