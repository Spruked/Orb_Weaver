...
* Ledger reconciliation correction: compound category labels were removed from the status contract. Crawl #4 now reports mutually exclusive category states: 2 verified, 7 partial, 2 blocked, 1 requiring runtime verification, and 10 runtime-only categories not crawl-verified. Every atomic item now carries the category’s conservative evidence state until finer stage evidence is available.
* Runtime promotion is explicit: a separate persisted `runtime_evidence` artifact must be `COMPLETE` and list each category ID before Live Test can promote that category. A deployment gate alone cannot change capability status.
